...
describe("court address storage", () => {
  const ENV_COURT = import.meta.env?.VITE_COURT_ADDRESS ?? "";
  const envConfigured = /^0x[0-9a-fA-F]{40}$/.test(ENV_COURT);

  it("round-trips through localStorage", () => {
    clearCourtAddress();
    setCourtAddress("0x000000000000000000000000000000000000dEaD");
    // A build-time env address always wins over the runtime override.
    const expected = envConfigured ? ENV_COURT : "0x000000000000000000000000000000000000dEaD";
    expect(getCourtAddress()).toBe(expected);
  });

  it("falls back to the empty string when nothing is configured", () => {
    clearCourtAddress();
    const value = getCourtAddress();
    expect(value === "" || value === ENV_COURT).toBe(true);
  });

  it("prefers the build-time env court when one is set", () => {
    if (!envConfigured) return; // nothing to assert on a blank .env
    clearCourtAddress();
    expect(getCourtAddress()).toBe(ENV_COURT);
  });
});
