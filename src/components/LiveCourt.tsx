...
import { checkCourt, type CourtHealth } from "../lib/courtHealth";
  ...
  return (
    <div className="rounded-2xl border border-iris-400/25 bg-iris-500/[0.06] p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <span className="font-mono text-[10px] text-white/40">{shortHash(tx.hash)}</span>
      </div>
...
export default function LiveCourt() {
  ...
  const [courtCompatible, setCourtCompatible] = useState<boolean | null>(null);
  const [courtHealth, setCourtHealth] = useState<CourtHealth | null>(null);
  const [gate, setGate] = useState<Record<number, SettlementStatus>>({});
  ...
  const loadDocket = useCallback(
    async (addr?: string) => {
      ...
      // ── hard pre-flight: never operate against a missing/stale court ──
      const health = await checkCourt(target);
      setCourtHealth(health);
      if (health.kind !== "ok") {
        const why =
          health.kind === "not-found"
            ? `court ${shortAddr(target)} is not deployed on Studionet (chain 61999) — the address may belong to a removed deployment or a different network. Deploy a fresh court or attach a valid one.`
            : health.kind === "old-shape"
              ? `court ${shortAddr(target)} was deployed from an older contract build (no appeal-window schema). Deploy the current contract and attach the new address.`
              : `court ${shortAddr(target)} answered but did not satisfy the AgentCourt schema — this is not an AgentCourt deployment.`;
        log(`✗ ${why}`, "err");
        log(`${health.message}`, "err");
        // drop the poisoned address so nothing (and no one) signs against it
        clearCourtAddress();
        setCourt("");
        setDisputes([]);
        setGate({});
        setCourtCompatible(false);
        setLoadingDocket(false);
        return;
      }

      log(`✓ court ${shortAddr(target)} is live on Studionet · security capabilities verified`, "ok");
      try {
        ...
        try {
          ...
          const compatible =
            Number(parsed.max_appeals) === 2 &&
            typeof parsed.appeal_bond === "string";
          setCourtCompatible(compatible);
          if (!compatible) {
            log(
              "this court does not expose the appeal-window and evidence-commitment capabilities required by the dApp — attach the current deployed court",
              "err",
            );
          }
        } catch {
          setFilingFee(null);
          setAppealBond(null);
          setCourtCompatible(null);
        }

        // appeal-window gate for every adjudged case
        const gates: Record<number, SettlementStatus> = {};
        for (const d of all) {
          if (d.status !== "ADJUDGED") continue;
          try {
            const raw = await c.readContract({
              address: target,
              functionName: "settlement_status",
              args: [d.id],
              stateStatus: "accepted",
            });
            gates[d.id] = JSON.parse(String(raw)) as SettlementStatus;
          } catch {
            /* older courts expose no settlement gate view */
          }
        }
        setGate(gates);
  ...
  const runTx = useCallback(
    async (
      ...
      setBusy(label);
      try {
        // Belt & braces: never ask the wallet to sign against a court that
        // failed its health check (missing deployment, stale attach, wrong
        // artifact). This is the guard that prevents "tx sent, no contract".
        if (court) {
          const health = await checkCourt(court);
          if (health.kind !== "ok") {
            log(
              `✗ ${label} blocked — ${health.kind === "not-found" ? "no contract at court address on Studionet" : "court schema lacks synchronous payout capability"}. No transaction was sent.`,
              "err",
            );
            setBusy(null);
            return;
  ...
  const deployCourt = useCallback(async () => {
    ...
    try {
      ...
      if (addr) {
        // Verify the freshly deployed court is actually readable before
        // saving it. A magic address that fails the health check would
        // poison every subsequent transaction.
        const health = await checkCourt(addr);
        setCourtHealth(health);
        if (health.kind === "ok") {
          setCourtAddress(addr);
          setCourt(addr);
          log(`✓ AgentCourt deployed at ${shortAddr(addr)} — verified live and saved as your court`, "ok");
        } else {
          log(`✗ deployed at ${shortAddr(addr)} but post-deploy health check failed: ${health.message}`, "err");
        }
      } else {
  ...
  const attachCourt = useCallback(async () => {
    if (!isValidAddress(attach)) {
      log("invalid address — expected 0x + 40 hex chars", "err");
      return;
    }
    const target = attach.trim();
    log(`verifying court ${shortAddr(target)} on Studionet…`);
    const health = await checkCourt(target);
    setCourtHealth(health);
    if (health.kind !== "ok") {
      log(`✗ cannot attach — ${health.message}`, "err");
      return; // never persist a dead address
    }
    setCourtAddress(target);
    setCourt(target);
    log(`✓ attached court ${shortAddr(target)}`, "ok");
    loadDocket(target);
  }, [attach, log, loadDocket]);
  ...
  const fileDispute = useCallback(async () => {
    ...
    if (!url) return log("task spec URL required", "err");
    ...
    // Immutable evidence commitment: hash the spec content now so a
    // later swap behind the same URL is provable by anyone. Commitment is
    // optional in the contract — empty string is permitted when the browser
    // cannot fetch the URL (CORS-blocked, localhost, private host).
    log("hashing task spec for the on-chain commitment…");
    const commitment = await commitmentFor(url);
    if (commitment) {
      log(`commitment ${commitment.slice(0, 22)}… recorded`, "ok");
    } else {
      log(
        "browser could not read the spec (CORS/offline/localhost) — filing without a client-side commitment; validators will render the URL directly at trial",
        "info",
      );
    }
    ...
    runTx("file_dispute", (c) =>
      c.writeContract({
        ...
        args: [url, commitment, defendant.trim()],
        value,
      }),
    );
  ...
  const submitDelivery = useCallback(
    async (id: number) => {
      ...
      if (!url) return log("delivery URL required", "err");
      if (!url.startsWith("https://")) {
        return log("delivery must be a public HTTPS URL — validators cannot access localhost, private hosts or ipfs://", "err");
      }

      log("hashing delivered work for the on-chain commitment…");
      const commitment = await commitmentFor(url);
      if (commitment) {
        log(`commitment ${commitment.slice(0, 22)}… recorded`, "ok");
      } else {
        log(
          "browser could not read the delivery (CORS/offline/localhost) — submitting without a client-side commitment; validators will render the URL directly at trial",
          "info",
        );
      }
      ...
      runTx(`submit_delivery #${id}`, (c) =>
        c.writeContract({
          ...
          args: [id, url, commitment, d.notes ?? ""],
        }),
      );
    },
  ...
  const acceptCase = useCallback(
    (id: number) => {
      runTx(`accept_verdict #${id}`, (c) =>
        c.writeContract({ address: court, functionName: "accept_verdict", args: [id] }),
      );
    },
    [court, runTx],
  );
  ...
  if (!w.hasWallet) {
    body = (
      <Gate
        ...
        body="The Studionet bench uses real escrow and live validators. Install MetaMask, then connect and add GenLayer Studionet."
      >
        <a
        ...
        <div className="flex flex-col justify-between rounded-2xl border border-gilt-400/25 bg-gradient-to-br from-gilt-400/[0.08] to-transparent p-6">
          <div>
            ...
            <p className="mt-2.5 text-[12.5px] leading-relaxed text-white/50">
              Deploy the intelligent dispute resolution contract directly from your wallet with a single transaction. Once consensus finalizes (~45s), your court is active on Studionet.
            </p>
        ...
        <div className="border-b border-white/[0.07] lg:col-span-4 lg:border-b-0 lg:border-r">
          <div className="border-b border-white/[0.07] p-5">
            ...
            <div className="mt-2.5 flex items-center gap-2">
              <span className="flex-1 truncate rounded-xl border border-white/10 bg-ink-900/80 px-3 py-2 font-mono text-[11px] text-gilt-200/90">
                {court}
              </span>
              <button
                onClick={() => {
            ...
            <div className="mt-3 flex items-center justify-between font-mono text-[10px] text-white/40">
              <span>
                ...
                {courtCompatible !== null && (
                  <span className={courtCompatible ? "text-aqua-300/60" : "text-ember-400"}>
                    · {courtCompatible ? "security checks ready" : "incompatible contract"}
                  </span>
                )}
              </span>
              <button
            ...
            <div className="flex items-center justify-between">
              ...
              {w.balance !== null && (
                <span className="flex items-center gap-1.5 font-mono text-[10px] text-white/40">
                  ...
                  <a href={STUDIONET.studio} target="_blank" rel="noreferrer" className="text-aqua-300/70 underline-offset-2 hover:underline" title="Studionet faucet — open Studio and use the account selector">
                    get GEN 💧
                  </a>
                </span>
              )}
                  ...
                  <div className="mt-4">
                    ...
                    {d.status === "EVIDENCE" && (
                      <button
                        ...
                        disabled={!!busy || courtCompatible !== true}
                        className="inline-flex items-center gap-2.5 rounded-xl bg-gradient-to-r from-gilt-400 to-iris-500 px-5 py-3 text-sm font-bold text-ink-950 shadow-[0_10px_35px_-12px_rgba(91,140,255,0.8)] transition-all disabled:cursor-not-allowed disabled:opacity-40"
                      >
                        {busy === `convene_trial #${d.id}` ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Gavel className="h-4 w-4" />}
                        {courtCompatible ? "Convene the trial — jury reasons live" : "Attach the current court to convene"}
                      </button>
                    )}

                    {d.status === "ADJUDGED" && (() => {
                      const g = gate[d.id] ?? null;
                      const canFinalize = g ? g.can_finalize : true;
                      const remaining = windowRemaining(g);
                      const iAccepted =
                        (me === d.plaintiff && d.accepted_plaintiff) ||
                        (me === d.defendant && d.accepted_defendant);
                      return (
                      <div className="space-y-3">
                        {/* appeal-window banner */}
                        <div className={`flex flex-wrap items-center gap-x-4 gap-y-1.5 rounded-xl border px-4 py-3 font-mono text-[10px] uppercase tracking-[0.14em] ${
                          canFinalize
                            ? "border-aqua-400/30 bg-aqua-400/[0.06] text-aqua-300"
                            : "border-iris-400/30 bg-iris-500/[0.07] text-iris-200"
                        }`}>
                          <span className="flex items-center gap-2">
                            <ShieldCheck className="h-3.5 w-3.5" />
                            {canFinalize ? "appeal window closed — settlement unlocked" : "appeal window open"}
                          </span>
                          {remaining && !canFinalize && <span className="text-white/45">{remaining}</span>}
                          <span className="text-white/35">
                            accepted: {d.accepted_plaintiff ? "P✓" : "P–"} {d.accepted_defendant ? "D✓" : "D–"}
                          </span>
                          <span className="text-white/30">appeals {d.appeals}/2</span>
                        </div>

                        <div className="flex flex-wrap gap-2.5">
                        {isParty && !iAccepted && (
                          <button
                            onClick={() => acceptCase(d.id)}
                            disabled={!!busy}
                            className="inline-flex items-center gap-2 rounded-lg border border-aqua-400/50 bg-aqua-400/10 px-4 py-2.5 font-mono text-[10.5px] uppercase tracking-[0.12em] text-aqua-200 transition-all hover:bg-aqua-400/20 disabled:opacity-50"
                          >
                            {busy === `accept_verdict #${d.id}` ? <LoaderCircle className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5" />}
                            Accept verdict · waive appeal
                          </button>
                        )}
                        {isParty && d.appeals < 2 && !iAccepted && (
                          <button
                        ...
                        <button
                          ...
                          title={canFinalize ? "" : "Both parties must accept, or the appeal window must elapse"}
                          className="inline-flex items-center gap-2 rounded-lg bg-gilt-400 px-4 py-2.5 font-mono text-[10.5px] font-semibold uppercase tracking-[0.12em] text-ink-950 transition-all hover:bg-gilt-300 disabled:cursor-not-allowed disabled:opacity-40"
                        >
                          {busy === `finalize #${d.id}` ? <LoaderCircle className="h-3.5 w-3.5 animate-spin" /> : <CircleDollarSign className="h-3.5 w-3.5" />}
                          {canFinalize ? "Finalize & settle escrow" : "Locked until window closes"}
                        </button>
                        </div>
                      </div>
                      );
  ...
  return (
    <div className="glass mt-6 overflow-hidden rounded-3xl">
      {/* health banner — surfaces bad deployments before any tx can be sent */}
      {courtHealth && courtHealth.kind !== "ok" && (
        <div className="border-b border-ember-500/30 bg-ember-500/[0.07] px-6 py-3.5">
          <p className="font-mono text-[10.5px] leading-relaxed text-ember-400">
            <span className="mr-2 font-bold uppercase tracking-[0.14em]">Court unreachable</span>
            {courtHealth.message}
          </p>
        </div>
      )}
